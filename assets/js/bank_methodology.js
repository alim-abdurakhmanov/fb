...
(function (window) {
    ...
    function initBankMethodology(opts) {
        ...
        function renderSummary() {
            ...
            const posClass = r.position === 'good' ? 'bm-pos-good'
            ...
            box.innerHTML = `
                ...
                <div class="bm-kpi">
                    ...
                    <div class="sub">версия ${assessment ? assessment.version : '—'} · <span class="bm-status-chip ${esc(status)}">${status === 'final' ? 'зафиксировано' : 'черновик'}</span></div>
                </div>
                <div class="bm-kpi">
                    <div class="label">Рейтинг</div>
                    <div class="value ${posClass}">${esc(r.rating || '—')}</div>
                    <div class="sub ${posClass}">${esc(r.position_label || '')}${r.hard_stop ? ' · стоп-фактор' : ''}</div>
                </div>`;
        }
        ...
        function renderAll() {
            ...
            const finInputsHtml = `
                <div class="bm-inputs">
                    ...
                    <div><label>Долг / выручка (коэф.)</label><input data-bm-fin="debt_to_revenue" inputmode="decimal" value="${esc(numOrEmpty(inputs.debt_to_revenue))}"></div>
                    <div><label>Отрасль</label>
                        <select data-bm-fin="industry">
                            <option value="default" ${inputs.industry === 'default' ? 'selected' : ''}>Обычная</option>
